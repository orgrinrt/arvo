...
| --- | --- | --- | --- |
| `` a_mod_core_shadows_the_bare_path.rs `` | 1 const assert | `` builds `` | builds |
...
| `` a_use_alias_shadows_the_bare_path.rs `` | 1 const assert | `` builds `` | builds |
...
| `` a_bare_mod_usize_confirms_the_original_premise.rs `` | 1 const assert | `` builds `` | builds |
...
| `` the_leading_double_colon_resists_both_shadows.rs `` | 1 const assert | `` builds `` | builds |
...
| `` the_use_alias_does_not_reach_the_leading_colon_path.rs `` | 1 const assert | `` builds `` | builds |
...
| `` the_leading_double_colon_resists_an_unaliased_use_of_core.rs `` | 2 const assert | `` builds `` | builds |
...
| `` the_leading_double_colon_resists_a_self_under_core.rs `` | 2 const assert | `` builds `` | builds |
...
| `` extern_crate_self_as_core_hijacks_the_absolute_path.rs `` | 1 const assert | `` builds `` | builds |
...
| `` the_raw_ident_extern_crate_self_hijacks_the_leading_colon_path.rs `` | 2 const assert | `` builds `` | builds |
...
| `` manifest_rename/user `` | 2 const assert | `` builds `` | builds |
...
| `` manifest_rename/user_of_the_crate_with_only_the_prelude `` | 2 const assert | `` builds `` | builds |
...
| `` manifest_rename/user_of_the_crate_with_an_empty_prelude `` | 2 array length | `` builds `` | builds |
...
| `` manifest_rename/a_std_dependent_of_the_crate_without_the_glob `` | 2 const assert | `` builds `` | builds |
...
| `` the_shipped_spelling_alone_is_silent.rs `` | 1 const assert | `` builds `` | builds |
| `` the_shipped_spelling_alone_is_silent.rs ``, item 1 flipped | const assert: `` <CHECK as F>::MAX != (1i128 << usize::BITS) - 1, `` | refused by that item alone | refused |
| `` the_bare_spelling_without_the_leading_colon_is_shadowed.rs `` | 1 const assert | `` refused with the bare spelling read the `mod core` shadow's 8 rather than the host's pointer width `` | refused |
| harness self-check | a vacuous check, no check, an unmessaged check | each flagged once | each flagged once |
