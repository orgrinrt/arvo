...
impl Nat for Z { const USIZE: usize = 0; }
impl<N: Nat> Nat for S<N> { const USIZE: usize = N::USIZE + 1; }
...
fn narrow_into<Out: Nat, In: Nat>(_: In) -> Out where In: AtLeast<Out>, Out: Default { Out::default() }

impl Default for Z { fn default() -> Z { Z } }
impl<N: Default> Default for S<N> { fn default() -> S<N> { S(core::marker::PhantomData) } }
