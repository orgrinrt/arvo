#!/usr/bin/env python3
"""
p3. Where the propagated grade stops being exact, which nobody has bounded.
...
def main():
    ...
    u2, c2, e2, t2 = sweep(
        S4, O("sub", L("x"), L("x")), range(0, 8), "dependency case for the check"
    )
    print(f"  the same counter on a repeated leaf is {c2}")
