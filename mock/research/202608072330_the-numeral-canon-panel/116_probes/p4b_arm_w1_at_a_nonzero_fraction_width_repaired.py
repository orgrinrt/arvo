#!/usr/bin/env python3
"""
p4b. The repair of p4: two-endpoint declarations, and an ambient span for the
     grid test.
...
def sweep(P, ts, label):
    ...
    for t in ts:
        ...
        per = [
            (a * P.s, b * P.s)
            for a in range(P.klo, P.khi + 1)
            for b in range(a, P.khi + 1)
        ]
        if len(names) == 2:
            combos = list(product(per, repeat=2))
        else:
            rng = random.Random(0xC0FFEE)
            combos = [tuple(rng.choice(per) for _ in names) for _ in range(4000)]
        for combo in combos:
            g = dict(zip(names, combo))
...
def main():
    ...
    print()
    P = Fx(4, 2, False, "wrap")
    ambient = [k * P.s for k in range(P.klo * 3, P.khi * 3 + 1)]
    for coarse_F in (2, 1, 0):
        step = Fraction(1, 2**coarse_F)
        grid = [v for v in ambient if (v / step).denominator == 1]
        bad = n = 0
        for a in grid:
            for b in grid:
                n += 1
                if P.R(ex("mul", P.R(a), P.R(b))) != P.R(ex("mul", a, b)):
                    bad += 1
        prod_on_grid = all(P.on_grid(a * b) for a in grid for b in grid)
        print(
            f"  operands on a grid of step {step}, ambient span {len(grid)} points: "
