...
selects an arm that is Pareto-dominated on the full coordinate set. It is not a
detector for a generator reading the wrong thing, and p2's prediction that it
would be was wrong.

Whether that event occurs at all is a property of the ARM SET, not of the
weighting. Arms drawn independently are almost never exactly tied, so a zero
weight moves the argmin without ever reaching a dominated arm, and the rung fires
on nothing. Arms sharing a coordinate exactly, which is the shape the committed
carrier table has twice over, are where the theorem earns its keep.

So `98`'s constraint stands and its predicate is narrower than the general
statement suggests: strict positivity buys the no-dominated-arm guarantee, the
guarantee is only reachable where two arms agree exactly on a coordinate, and it
buys nothing whatever against a generator computing the wrong argmin.
"""
    )
