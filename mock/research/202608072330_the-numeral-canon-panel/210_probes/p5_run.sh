...
src="$here/p5_contextual_formulation.rs"
out="$here/p5_out"
rm -rf "$out"
mkdir -p "$out"
RUSTC="${RUSTC:-rustc}"
...
if ! $RUSTC +nightly-2026-05-28 --edition 2021 -O -o "$out/p5" "$src" 2>"$out/build.log"; then
    echo "*** p5 DID NOT COMPILE, no number below means anything ***"
    cat "$out/build.log"
    exit 1
fi
echo "### compiled clean"
