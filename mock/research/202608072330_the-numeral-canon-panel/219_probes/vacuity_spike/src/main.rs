...
use arvo_checks::{citation, comments, corpus, load, obligation, predicate, provenance, shape, Registry};
...
fn main() {
    ...
    macro_rules! arm {
        ($name:literal, $f:path) => {{
            let e = $f(&empty).len();
            let r = $f(&real).len();
            let verdict = if r == 0 { vacuous += 1; "VACUOUS" } else { guarded += 1; "guarded" };
            println!("  {:<8} {:<58} empty {:>3}   canon {:>3}", verdict, $name, e, r);
        }};
    }

    println!("=== the 25 arms that take a Registry ===");
    arm!("citation::line_citations_into_living_ledgers", citation::line_citations_into_living_ledgers);
    arm!("citation::citations_with_no_target", citation::citations_with_no_target);
    arm!("comments::comments_counting_their_own_rows", comments::comments_counting_their_own_rows);
    arm!("obligation::obligation_edges_naming_nothing", obligation::obligation_edges_naming_nothing);
    arm!("obligation::obligation_edges_from_an_untiered_namespace", obligation::obligation_edges_from_an_untiered_namespace);
    arm!("obligation::obligations_whose_only_route_is_closed", obligation::obligations_whose_only_route_is_closed);
    arm!("obligation::preconditions_from_a_namespace_that_cannot_establish_one", obligation::preconditions_from_a_namespace_that_cannot_establish_one);
    arm!("obligation::unanswered_obligations_carrying_a_precondition", obligation::unanswered_obligations_carrying_a_precondition);
    arm!("predicate::undeclared_dimensions", predicate::undeclared_dimensions);
    arm!("predicate::repeated_dimensions", predicate::repeated_dimensions);
    arm!("provenance::standing_claims_more_arrivals_than_it_cites", provenance::standing_claims_more_arrivals_than_it_cites);
    arm!("provenance::a_proposal_resting_only_on_a_consolidation", provenance::a_proposal_resting_only_on_a_consolidation);
    arm!("provenance::an_imposition_resting_on_an_instrument", provenance::an_imposition_resting_on_an_instrument);
    arm!("shape::rulings_with_no_verbatim", shape::rulings_with_no_verbatim);
    arm!("shape::refusals_without_an_instead", shape::refusals_without_an_instead);
    arm!("shape::measured_without_evidence", shape::measured_without_evidence);
    arm!("shape::measurements_resting_on_an_unusable_instrument", shape::measurements_resting_on_an_unusable_instrument);
    arm!("shape::predicate_disagrees_with_the_sentence_kind", shape::predicate_disagrees_with_the_sentence_kind);
    arm!("shape::stamps_from_an_unratified_ruling", shape::stamps_from_an_unratified_ruling);
    arm!("shape::a_term_defined_twice", shape::a_term_defined_twice);
    arm!("shape::definitions_with_no_term", shape::definitions_with_no_term);
    arm!("shape::rows_restating_a_retired_claim", shape::rows_restating_a_retired_claim);
    arm!("shape::rows_with_no_keywords", shape::rows_with_no_keywords);
    arm!("shape::notes_claiming_an_empty_field_that_is_not", shape::notes_claiming_an_empty_field_that_is_not);
    arm!("shape::retirements_too_short_to_find", shape::retirements_too_short_to_find);
    ...
    macro_rules! dirarm {
        ($name:literal, $f:path) => {{
            ...
            let r = $f(&panel).len();
            let verdict = if r == 0 { vacuous += 1; "VACUOUS" } else { guarded += 1; "guarded" };
            println!("  {:<8} {:<58} missing {:>3}   panel {:>3}", verdict, $name, e, r);
        }};
    }
    dirarm!("corpus::unprefixed_archive_citations_in_living_ledgers", corpus::unprefixed_archive_citations_in_living_ledgers);
    dirarm!("corpus::archive_citations_naming_nothing", corpus::archive_citations_naming_nothing);
    dirarm!("corpus::line_citations_into_the_registry", corpus::line_citations_into_the_registry);
    dirarm!("corpus::probes_reading_another_tree", corpus::probes_reading_another_tree);
    dirarm!("corpus::line_citations_into_living_ledgers_in_prose", corpus::line_citations_into_living_ledgers_in_prose);

    println!();
    println!("of 30 arms: {vacuous} say nothing on either input, {guarded} would notice");
    assert!(guarded > 0, "control: if nothing fires on the real corpus this measures its own wiring");
}
