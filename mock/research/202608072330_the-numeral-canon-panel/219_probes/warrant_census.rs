    ...
    for row in rows {
        ...
        for (field, entry) in &row.entries {
            ...
            if slug.is_empty() || !slug.chars().all(|ch| ch.is_ascii_lowercase() || ch == '_') {
                c.dirty_slugs
                    .push(format!("{addr} => slug side is `{slug}`"));
            }
            ...
            if WARRANTS
                .iter()
                .any(|w| values == *w || values.starts_with(&format!("{w} ")))
            {
        ...
        if has_any && has_bounded {
            c.mixed_rows.push(format!("{}::{}", row.namespace, row.id));
            *c.kind_of_any_rows
...
fn report(label: &str, c: &Census) {
    ...
    println!(
        "Q1  entries whose values are the axis universal: {}",
        c.any_entries
    );
    ...
    println!(
        "Q6  slug sides that are not a bare lowercase slug: {}",
        c.dirty_slugs.len()
    );
    ...
    println!(
        "Q7  axes writing the universal as a bare `any`: {:?}",
        c.bare_any
    );
    println!(
