...
fn run(label: &str, rows: &[Row]) -> Out {
    ...
    for row in rows {
        ...
        for entry in &row.entries {
            ...
            let n_all = clauses(values)
                .iter()
                .filter(|c| !binds(c).is_empty())
                .count();
            if n_comma > 1 {
                o.multi_comma += 1;
            }
    ...
    println!(
        "R2  multi-binding, census rule (comma only): {}",
        o.multi_comma
    );
    println!(
        "R2  multi-binding, corrected rule (comma or `and`): {}",
        o.multi_with_and
    );
    ...
    println!(
        "R3  sentence_kind = theorem rows and their width span: {}",
        o.theorem.len()
    );
...
fn control() -> bool {
    ...
    check(
        "multi-binding the census rule misses",
        o.missed_multi.len(),
        1,
    );
