...
| `p6` | citation checker | opens every `file:line` in this document and tests its content against a word it must contain: **34 checked, 0 failures**, and a cross-check confirms the probe's table and the document's citations are the same set. It failed on three separate runs, on a range that opened mid-sentence, on three citations absent from its own table, and on a range transcribed one line off, and all three are recorded in section 12 rather than repaired silently |
