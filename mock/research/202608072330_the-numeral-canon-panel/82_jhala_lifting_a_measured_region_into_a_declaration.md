...
**And re-verified after the formatter, not before it.** The commit hook runs rustfmt on staged sources, so
every probe's committed text differs from the text I ran. `cl-claim-sketch-discipline.md` records the exact
failure that causes: a changelist naming a check that passed before the autofix and fails against what
actually landed. So `p1`, `p2`, `p5`, `p6`, `p8`, `p10` and `p12` were rebuilt from their committed sources
after the hook had touched them and diffed against the transcripts beside them. All seven reproduce
byte-for-byte. `p3`, `p4`, `p7`, `p9` and `p11` are compile-outcome and assembly probes whose transcripts
are generated by committed scripts, and `p8` re-running clean re-establishes the compile outcomes for `p3`
and `p7`.
