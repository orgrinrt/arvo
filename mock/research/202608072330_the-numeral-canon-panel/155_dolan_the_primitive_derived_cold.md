...
## Addendum: two things found after the reconciliation above was written, both worth more than a footnote

### A search error of my own, caught and corrected

My "what I read" section above understated `OPTIONS.md`. I scanned it with `grep -n "^## Q1[0-9]\b\|^## Q5[0-9]\b"`, two hashes, and reported reading "Q16 in full... plus its header index and its closing 'not yet asked' section," treating Q17 as the last real entry before that tail. **`OPTIONS.md`'s entries from Q18 onward are headed `###`, three hashes, not two**, so my search silently skipped 47 entries, Q18 through Q64, including `Q52`, titled "What a primitive is, and two convergences that did not survive being checked." I found this only because a later cross-check (looking for `Q64`, which `153` section 5 cites by number) returned nothing against my first pattern and forced me to grep the file's actual heading structure rather than trust my first pass.

**Q52 confirms every substantive claim in this file's reconciliation above rather than correcting any of it.** It states, independently in the register's own voice: the law set/refinement reversal is `111`'s and is one expert; the "0 of 48" count is dead and the conclusion survives on three other instruments (naming the same three I named: `110` TEST 2, `109` P2, `90` R3); the number-system convergence is one expert, not two, because `109` conceded rather than corroborated; `109`'s const-eval blocker is real and its conclusion is false, with the same three carriers and the same `satfold-shared` citation I read at source; the signature claim saturates at the literal, with the same 165-versus-148 figures; the two degeneracies collapse under constant injection, 144 to 0; and what remains open is the same item I already flagged (whether the refinement is a member of the strategy pair's first component, which `112` and `111` section 18 settle as no, after Q52 was written and left it open). **So the correction is entirely to my own coverage claim, not to my content.** I did not cite anything from OPTIONS.md that I got wrong; I simply undercounted what I had read, and reading the rest turned up agreement rather than a defect. Recorded because a wrong coverage statement is exactly the kind of thing `RULES.md` says to check by opening the citation rather than trusting the pattern that produced it, and my pattern was the thing at fault here, not any claim built on it.

I also read `Q53` through `Q59` (the realisation-map and rounding-axis topics' own open items) at this pass, briefly. None bears on the primitive question directly; `Q55`, the accumulator-width collapse under the shipped `warm-clamp` guard being unpriced, is the same unpriced-consumer-attached item I already named in my "what remains open" section from the bench-evidence side (`warm-clamp-shared`'s interior-safety predicate), and Q55 confirms it is still unpriced as of the register's own account.

### The sibling cold derivation this unit dispatched alongside mine is incomplete, as of this writing

`153_dispatcher_note_opening_the_ninth_unit_on_a_cold_return.md` and `156_checkpoint_nine_the_queue_for_ops_seat.md`, neither in my curated reading list, record the reason this dispatch exists at all: topic five (`109` through `114`) never received a canon candidate, a ledger, a signature round, or an independent check, because the unit moved to the refinement half before any of that happened, and `AGREEMENTS.md` has a section for every other topic and none for this one. `153` section 4 states the remedy directly: reopen with **a fresh blind cold pair on the same question**, because the panel's bar is three independent instances and topic five has two (`109`, `110`), and a pair that has read neither is what the question is short of.

I am one half of that pair. The other is `154_kiselyov_the_primitive_derived_cold.md`. As of my reading it, `154` consists of its header, its canon gate (passed, flagging that "primitive" carries at least three incompatible senses in the governing material, which is a genuinely useful independent framing of the exact ambiguity this dispatch was asked to resolve) and a thorough, independently-run test gate (per-crate `cargo test` results for all thirteen `-shared` crates, a mechanical scan of all 123 parsed test bodies for missing assertions, and two real defects named: a structurally-unfailable stress test costing wall-clock time on every run, and the suite's total runtime risking it stops being run at all). **It stops there.** No "answer, stated first," no derivation, nothing past the gates. Its file is 125 lines, short by the standard every other cold derivation in this topic set. Its git history shows one commit, at 06:32:33, for the file and one probe together; the working tree is otherwise clean, so it is not sitting half-staged, it is a member whose dispatch had not yet produced a derivation at the time I checked.

I did not read it as a phase-two source (nothing in it yet exists to read past the gates), and I am not attempting to resume, extend, or complete it: it is not mine to touch, per the same discipline that governs a shared clone or a live member's in-progress work, and per `153` section 1's own finding that a member handle is addressable only from the session that dispatched it, which is not necessarily this one. **I record its state because the coordinator reading this file needs to know, before treating my reconciliation as "the fresh pair's verdict," that only one half of the pair has reached a verdict.** Whether `154` lands on something compatible with `112`'s offered statement without having read it, per `153` section 4's own framing of what would count as the strongest evidence available, is a question this file cannot answer, because `154` had not answered it as of when I looked.

**What this means for how the rest of this file should be read.** My own phase-one derivation is a genuine third independent instance on the question, arrived at blind, and my phase-two reconciliation is a genuine (if solitary, for now) test of whether a fresh reading confirms `109` through `114`'s converged shape. It found strong, specific confirmation with one substantial gap (I never found the refinement blind) and no contradiction. That is one member's contribution to the bar of three this unit was dispatched to reach, not the bar reached. The register entry topic five still needs, per `153` and `156`, is a ledger, a revision, and an independent check, none of which this file is, and none of which should be inferred from it existing.
