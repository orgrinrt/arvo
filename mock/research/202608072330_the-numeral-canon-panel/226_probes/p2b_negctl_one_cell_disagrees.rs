//! p2b NEGATIVE CONTROL. p2 with one cell of table B changed: at W = 13 order B
//! says the Cold carrier is `u64` where order A says `u32`. This MUST refuse. If
//! it compiles, `the_two_orders_agree` is not checking anything and p2 proves
//! nothing.
//!
//! Its own first draft edited BOTH tables, because the two lines are textually
//! identical, so the orders still agreed and it compiled. A control that moves
//! both sides of an equality is not a control. Only the second occurrence is
//! changed here.
//!
