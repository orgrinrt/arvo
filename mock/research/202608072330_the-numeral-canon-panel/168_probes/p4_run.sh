...
set -e
SRC=p4_the_collapse_survives_the_profile.rs
SYMS="affine_deferred affine_eager blocked_deferred blocked_eager"
...
for spec in "default:-Copt-level=3 -Clto=off -Ccodegen-units=16" \
  ...
  rustc --crate-type cdylib $flags -o "libp4_$name.dylib" "$SRC" 2>/dev/null
  otool -tV "libp4_$name.dylib" > "p4_${name}_linked.s"
  echo "profile=$name  ($flags)  [cdylib, post-LTO disassembly]"
  for f in $SYMS; do
    addr=$(nm -g "libp4_$name.dylib" 2>/dev/null | awk -v s="_$f" '$3==s {print $1}' | head -1)
    fold=$(nm -g "libp4_$name.dylib" 2>/dev/null | awk -v a="$addr" -v s="_$f" '$1==a && $3!=s && $3 ~ /^_[a-z]/ {printf "%s ", $3}')
    body=$(awk -v F="_$f" '$0 ~ "^" F ":" {inf=1; next} inf && /^_[a-zA-Z]/ {inf=0} inf {print}' "p4_${name}_linked.s")
    lines=$(printf '%s\n' "$body" | grep -c . || true)
  ...
  ctrl=$(awk -v F="_affine_deferred" '$0 ~ "^" F ":" {inf=1; next} inf && /^_[a-zA-Z]/ {inf=0} inf {print}' "p4_${name}_linked.s" | grep -cE '[ \t,][vq][0-9]+([ \t,]|$)' || true)
  if [ "$ctrl" -eq 0 ]; then
    echo "  *** p4 note: even the deferred arm did not vectorise at this profile."
    echo "  *** Read p5, which carries the controls."
  else
    echo "  note: p4 has no projection-free control of its own; its deferred arm is the"
    echo "  reference, and p5 is where the control lives."
  fi
  echo
done
