//! p07c. Following rustc's own suggestion from p07b: declare the nat's value as
...
//!       --emit=metadata -o out/p07c.meta p07c_pin_const_to_nat_type_const.rs 2> out/p07c.log
...
impl<T: Nat> Nat for D0<T> {
    type const V: u32 = 2 * T::V;
}
impl<T: Nat> Nat for D1<T> {
    type const V: u32 = 2 * T::V + 1;
}
