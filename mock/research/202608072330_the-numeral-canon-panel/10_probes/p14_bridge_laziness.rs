//! P14. Does a bridge row cost anything when its width is never written?
//! op at `127b:36-50` asked that "only used widths realise on const time".
//! Test: add a row whose nat has NO `Nat`, `Len`, `Add` or `Container` impl at
//! all, so the ladder cannot run on it. If the crate still compiles, an unused
//! row is registered and never realised, which is the property he asked for.
//! (was P12.) `137`'s full construction (p6 + p7), unchanged in mechanism, with three
...
// a nat with no ladder impls whatsoever
pub struct Unrealisable;
pub struct Never;
impl ToNat<Never> for Idx<9999> {
    type N = Unrealisable;
}
