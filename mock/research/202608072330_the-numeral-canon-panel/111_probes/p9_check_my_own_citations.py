...
CITES = [
    ...
    # ---- added with the reply to 112
    (f"{PANEL}/108_lamport_the_pair_attacked.md", 825,
     "An axis belongs here if there is **any** reachable chain",
     "108's membership criterion, the sentence after the one 111 and 112 used"),
    (f"{PANEL}/108_lamport_the_pair_attacked.md", 824,
     "cannot recover it from the bits, so every consumer of that value",
     "the middle sentence, whose reason 111 used and whose consequence 112 used"),
    (f"{PANEL}/112_leijen_where_the_refinement_lives.md", 110,
     "two consumers holding different",
     "112's agreement claim, which p2b's table does not measure"),
    (f"{PANEL}/112_leijen_where_the_refinement_lives.md", 1029,
     "exact only on left-nested",
     "F112-6's 'only', refuted by its own p3b output"),
    (f"{PANEL}/112_leijen_where_the_refinement_lives.md", 1039,
     "no node-wise rule reaches",
     "F112-7's unreachability claim, bounded by R2 to one-sided declarations"),
    (f"{PANEL}/112_leijen_where_the_refinement_lives.md", 1042,
     "A repeated leaf is not sufficient",
     "F112-8, the half of the correction 112 did not carry into F112-6"),
    (f"{PANEL}/112_leijen_where_the_refinement_lives.md", 1142,
     "Every declared extent in every sweep of this file is one-sided",
     "F112-23, which R2 connects to F112-7"),
    (f"{PANEL}/112_probes/p3b_output.txt", 21,
     "(x + y) - z, SIGNED sat W=4",
     "the mixed-operation row that is exact"),
    (f"{PANEL}/112_probes/p3b_output.txt", 16,
     "(x + y) - x, SIGNED sat W=4",
     "the repeated-leaf row that is exact"),
]
