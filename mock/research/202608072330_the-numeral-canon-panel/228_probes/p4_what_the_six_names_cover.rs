...
fn floor_(x: i128, d: i128) -> i128 {
    x.div_euclid(d)
}
fn ceil_(x: i128, d: i128) -> i128 {
    -((-x).div_euclid(d))
}
fn toward_zero(x: i128, d: i128) -> i128 {
    x / d
}
...
fn main() {
    ...
    for (label, vocab) in [
        (
            "A: half_up = floor(x + 1/2), the corpus's instruments",
            &reading_a,
        ),
        ...
        println!(
            "{:<22} {:<18} {:<12} {}",
            "reference rule", "named in", "covered?", "by which ratified name"
        );
    ...
    println!(
        "{:<22} {:<12} {}",
        "reference rule", "covered?", "by which ratified name (reading A)"
    );
    ...
    if differ_signed && agree_unsigned {
        println!(
            "  C3 EXPECTED ok: the readings differ signed and agree on the non-negative domain"
        );
    } else {
    ...
    let a_unsigned = reading_a
        .iter()
        .any(|(_, vf)| same(away_from_zero, *vf, s, 0, hi));
    let a_signed = reading_a
        .iter()
        .any(|(_, vf)| same(away_from_zero, *vf, s, lo, hi));
