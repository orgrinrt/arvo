//! 68 probe p3: MUTANT of 65_probes/derive_validate_erase.rs.
//!
//! One deliberate lie is introduced: the storage representation Biased4InC8
//! DECLARES a coverage window of [-100, 100] (and REDUNDANT = true), while its
//! encode/decode functions cover exactly [-3, 12] as before. The question this
//! mutant answers: can 65's validation suite detect an OVERSTATED declaration?
//!
//! Expected per 68's analysis: it compiles clean. The coverage check
//! covers(rep, demand) only fails when a declaration UNDERSTATES; the
//! round-trip check quantifies over the demand window through the real
//! functions and never consults the declaration; REDUNDANT is read by nothing.
//! So the declared window is a claim no check constrains from above, and the
//! guarantee that actually holds is carried entirely by the round-trip over
//! the demand window, not by the declaration.
...
impl Representation for Biased4InC8 {
    type C = C8;
    const LO: i64 = -100; // LIE: decode can never produce a value below -3
    const HI: i64 = 100; // LIE: decode can never produce a value above 12
    const REDUNDANT: bool = true; // LIE: read by nothing, constrains nothing
}
