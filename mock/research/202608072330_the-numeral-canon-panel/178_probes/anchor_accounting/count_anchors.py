#!/usr/bin/env python3
"""178 anchor accounting. Counts the citation anchors in the fourteen source files this
...
CAND = "178_leroy_the_restoration_pass.md"
...
def main():
    ...
    print(f"\nunique line anchors across the 14 sources : {len(src_anchors)}")
    print(f"unique probe paths across the 14 sources  : {len(src_probes)}")
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "dropped_anchors.txt"), "w") as f:
        f.write("# anchors present in the 14 source files and not carried into 178.\n")
    ...
    print("\n-- controls --")
    pos = "174:384" in ca
    print(f"positive control (174:384 found in candidate): {'PASS' if pos else 'FAIL'}")
