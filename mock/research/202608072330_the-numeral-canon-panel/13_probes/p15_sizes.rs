...
// ---- multiply: output widths are nat sums. Does it re-enter the bridge? ----
pub trait MulShape<R> {
    type Out;
}
impl<I1, F1, I2, F2, Sign, S> MulShape<Numeral<I2, F2, Sign, S>> for Numeral<I1, F1, Sign, S>
where
    Pair<I1, F1>: Shape,
    Pair<I2, F2>: Shape,
    I1: Add<I2>,
    F1: Add<F2>,
    Pair<Sum<I1, I2>, Sum<F1, F2>>: Shape,
{
    type Out = Numeral<Sum<I1, I2>, Sum<F1, F2>, Sign, S>;
}
pub type Prod<A, B> = <A as MulShape<B>>::Out;

// one multiply: the result is NAMABLE at the surface, because 26 and 6 are
// literals the table already carries
fn _one_multiply()
where
...
// four multiplies: 13 -> 26 -> 52 -> 104 -> 208 integer bits, and 3 -> 6 -> 12
// -> 24 -> 48 fractional. 104 and 208 are ABOVE the bridge table's last row (64).
pub type M2 = Prod<Money, Money>;
pub type M4 = Prod<M2, M2>;
pub type M8 = Prod<M4, M4>;
pub type M16 = Prod<M8, M8>;

// the container for a width the bridge never saw
pub const SZ_M16: usize = core::mem::size_of::<M16>();
pub const SZ_M8: usize = core::mem::size_of::<M8>();
pub const SZ_M4: usize = core::mem::size_of::<M4>();

fn main() {
    println!(
        "StrHandle  UInt<5>        size={} align={}",
        core::mem::size_of::<StrHandle>(),
        core::mem::align_of::<StrHandle>()
    );
    println!(
        "Money      UFixed<13,3>   size={} align={}",
        core::mem::size_of::<Money>(),
        core::mem::align_of::<Money>()
    );
    println!(
        "M2         w=32           size={}",
        core::mem::size_of::<M2>()
