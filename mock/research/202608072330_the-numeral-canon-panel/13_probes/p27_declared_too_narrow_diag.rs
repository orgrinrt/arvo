...
// (the reverse table is DELETED in arrangement D)
pub trait Repr {
    type R;
} // L<K> -> structural nat
...
// ---- Arrangement D: the output width is DECLARED by the consumer, and the
// type system checks it is wide enough. No reverse table, so no ceiling, and
// the width types stay `L<K>` so diagnostics carry the consumer's numbers.
#[diagnostic::on_unimplemented(
    message = "the declared output width is narrower than the product needs",
...
pub type Product = UFixed<26, 6>;
pub type Roomy = UFixed<32, 8>;
pub type TooNarrow = UFixed<20, 6>;

// wide enough, and exactly enough
fn _ok()
where
    Money: MulInto<Money, Product> + MulInto<Money, Roomy>,
{
}

// the declared output is too narrow: what does the consumer read
fn _too_narrow()
where
