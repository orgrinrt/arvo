...
// ============================================================================
// b01 ADDITION. Everything above this line is `10_probes/p12_improved_full.rs`,
// copied unmodified. The question below is not about the diagnostics `10` added.
//
// The ladder is total: it maps ANY width to a container with no table. The
// bridge is a table of thirteen rows. `mul` requires the OUTPUT width to have a
// bridge row (`Idx<OI>: ToNat<M>`, line 205 above). So the algebra is closed
// only over widths the table happens to contain.
//
// Note what is already visible in the table above: rows 26 and 6 exist only
// because 13 times 13 needs them. The author had to close the table by hand for
// one multiply.
//
// Below: two numerals whose widths ARE in the table, multiplied. 24.8 times
// 24.8 is 48.16. Neither 48 nor 16-as-a-fraction-width is reachable, and 48 is
