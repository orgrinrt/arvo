...
bridge! {
    ...
    // b02: the two rows b01 was missing, added by hand, with their binary
    // expansions computed by hand. This is what "one line per width" costs.
...
// ============================================================================
// b02. Everything above is `10_probes/p12_improved_full.rs` with ONE bridge row
// added: width 48, whose binary expansion I had to compute by hand to write it.
//
// That row closes b01's failure. The site below now compiles.
//
// Then the next multiply reopens it, one octave up. This is the point: the
// widths reachable from a declared set under multiplication are unbounded
// (24, 48, 96, 192, ...), so NO finite table is closed under the algebra, at
// any size. A table is a ceiling on the laws, in the same sense a fixed-width
// carrier was a ceiling on the storage.
// ============================================================================

// closed by the new row
pub fn b02_now_compiles(a: UFixed<24, 8, Hot>, b: UFixed<24, 8, Hot>) -> Fixed<48, 16, Hot> {
    mul(a, b)
}

// the next octave, which the new row does not reach
