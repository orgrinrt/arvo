...
/// LEAK 1: a public constructor taking the container.
impl<const W: usize, S> UFixed<W, S> {
    pub const fn from_container(raw: u16) -> Self {
        UFixed {
            raw,
            _s: core::marker::PhantomData,
        }
...
/// LEAK 2: a From impl, which is the same hole wearing a trait.
impl<const W: usize, S> From<u32> for UFixed<W, S> {
    fn from(v: u32) -> Self {
        UFixed {
            raw: v as u16,
            _s: core::marker::PhantomData,
...
/// LEAK 3: a public field on a neighbouring type that carries the numeral's storage.
/// This is the one that never appears near the guarantee it breaks.
pub struct Column<const W: usize, S> {
    pub backing: [u16; 8],
    _s: core::marker::PhantomData<S>,
}
