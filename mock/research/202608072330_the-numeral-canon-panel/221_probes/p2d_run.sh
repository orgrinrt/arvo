...
echo
if ! $RUSTC +nightly-2026-05-28 --edition 2021 -O -o "$out/p2d" "$src" 2>"$out/build.log"; then
    echo "*** p2d DID NOT COMPILE, no number below means anything ***"
    cat "$out/build.log"
    exit 1
fi
echo "### compiled clean"
