//! p1: the same declaration, built for more than one pointer width.
//!
...
impl Format for PlatformSigned {
    ...
    type Quantum = Constant<0>;
    type Slots = Signed<{ isize::BITS }>;

    const PHASE: Phase = Phase::ZERO;
}

/// `2^W - 1` computed wide, so the shift itself cannot refuse at `W = 64`.
const TOP: i64 = ((1i128 << usize::BITS) - 1) as i64;
...
const _: () = assert!(
    WIDTH.count() == usize::BITS,
    "the width coordinate is the pointer width"
);
const _: () = assert!(
    COUNT.count() as i128 == 1i128 << usize::BITS,
