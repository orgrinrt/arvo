# 233. Kiselyov: the nine rounding entries, read off their instruments

Seat 233. The question is what nine predicate entries state on the rounding
axis, in the six names `ruling::the_ambiguous_rounding_word_is_retired_for_six_explicit_names`
closed the vocabulary at, and what the evidence for each is.

Two probes, both committed in `233_probes/` with their raw output beside them,
both carrying predictions written into the source before the first run and both
carrying the case that must fail. Every conclusion below names the instrument
that established it and the domain that instrument actually ran over.

## 0. The two gates

**Canon gate: passed.** Checked against the two ratified rows the brief names as
governing, both read in full from `mock/registry/ruling.toml` before anything
else.

`ruling::the_ambiguous_rounding_word_is_retired_for_six_explicit_names` closes
the vocabulary at `toward_zero`, `floor`, `ceil`, `half_up`, `half_even`,
`stochastic`, retires `truncation` and `trunc` in both spellings, and records
that bit truncation of a two's complement value is floor. Nothing below reopens
that. Where I say the set is short a name I say it as a finding against the set
with the measurement attached, which is what
`question::is_the_rounding_vocabulary_complete_at_six` exists to receive, and I
do not make the call.

`ruling::the_panel_finishes_the_canon_without_him` puts every remaining canon
question with the panel and forbids routing anything to op. Two `question` rows
reserve exactly the two calls this seat touches, both with `decider = "panel"`,
so the work is licensed and nothing here is parked awaiting him.

**Test gate: run, and it has one red that is documented and deliberate.**
`cargo mock test` reports 576 passed, 0 failed, 13 ignored across seven of eight
trees. The eighth, `mock/benches`, fails on four variant manifests. That is not
mine and is not a defect: `mock/benches/variants/fnv1a/Cargo.toml:1-12` carries a
marker saying the variant has not built since 2026-08-08, that it inherits
workspace dependencies naming deleted crates, that it is kept because its
committed results are evidence the panel cites, and that it is not repaired
because "an arm invented to make a manifest parse measures nothing". That is a
catalogue-red stated in the right place, and the three other failures are the
same shape.

I also read the suite of the one instrument this seat depends on,
`mock/tools/rounding-vocabulary/src/tests.rs`, 416 lines, because the enumeration
of my nine comes out of it. It is a real suite. It carries negative controls that
can fire (`a_canon_with_no_rounding_entry_is_inconclusive_rather_than_clean`,
`a_word_merely_containing_a_retired_one_is_not_caught`,
`control_a_canon_spelling_every_mode_correctly_is_clean`), it asserts that a
ruling which dropped one of the six stops the run and names which
(`a_ruling_that_dropped_a_name_stops_the_run_and_says_which`), and I found
nothing tautological in it. The tool hardcodes the six at
`mock/tools/rounding-vocabulary/src/lib.rs:51` and then checks at line 185 that
every one of them appears in the ratified row, going inconclusive if not, so it
is pinned to the canon rather than floating beside it. I have one substantive
finding against the tool and it is in section 9, not here: the suite is sound and
one of the tool's classifications is not.
