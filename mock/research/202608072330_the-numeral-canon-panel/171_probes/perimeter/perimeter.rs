...
#[inline(never)]
fn mk_wide(a: i32, b: i32) -> i64 { a as i64 + b as i64 }
#[inline(never)]
fn mk_narrow(a: i32, b: i32) -> i32 { a.wrapping_add(b) }

// The same quantity behind an OPAQUE type: the caller cannot name i64 or i32.
trait Carry: Copy { fn finish(self, c: i32) -> i32; }
impl Carry for i64 { fn finish(self, c: i32) -> i32 { (self - c as i64) as i32 } }
impl Carry for i32 { fn finish(self, c: i32) -> i32 { self.wrapping_sub(c) } }

#[inline(never)]
fn mk_wide_opaque(a: i32, b: i32) -> impl Carry { a as i64 + b as i64 }
#[inline(never)]
fn mk_narrow_opaque(a: i32, b: i32) -> impl Carry { a.wrapping_add(b) }

// C-F's pair: both implementations bind the intermediate at the SAME type.
#[inline(never)]
fn mk_same_a(a: i32, b: i32) -> i64 { a as i64 + b as i64 }
#[inline(never)]
fn mk_same_b(a: i32, b: i32) -> i64 { (a as i64).wrapping_add(b as i64) }
...
fn main() {
    ...
    println!("   size_of_val  wide {}  narrow {}  distinguishes: {d1}", size_of_val(&tw), size_of_val(&tn));
    ...
    println!("   size_of_val  wide {}  narrow {}  distinguishes: {d2}", size_of_val(&ow), size_of_val(&on));
    println!("   both still compute the same final value: {} == {} -> {}",
        ow.finish(c), on.finish(c), ow.finish(c) == on.finish(c));
    ...
    println!("   size_of_val  {}  {}  distinguishes: {d3}   (must be false)", size_of_val(&sa), size_of_val(&sb));
    println!("   values equal: {}", sa == sb);

    println!();
    println!("C-H  size_of_val distinguishes at least one pair: {}", d1 || d2);
    println!();
    println!("VERDICT");
    println!("  a binding at a differing type admits a distinguishing context: {}", d1 && d2);
