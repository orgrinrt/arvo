...
pub trait NatVal { const VAL: u32; }
impl NatVal for Z { const VAL: u32 = 0; }
impl<N: NatVal> NatVal for Su<N> { const VAL: u32 = N::VAL + 1; }
...
impl<N: PosVal> PosVal for TwiceP1<N> { const VAL: u64 = 2 * N::VAL + 1; }

pub trait Inc { type Out; }
impl Inc for One { type Out = Twice<One>; }
impl<N> Inc for Twice<N> { type Out = TwiceP1<N>; }
impl<N: Inc> Inc for TwiceP1<N> { type Out = Twice<<N as Inc>::Out>; }

pub trait Log2Ceil { type Out; }
impl Log2Ceil for One { type Out = Z; }
impl<N: Log2Ceil> Log2Ceil for Twice<N> { type Out = Su<<N as Log2Ceil>::Out>; }
impl<N> Log2Ceil for TwiceP1<N>
where N: Inc, <N as Inc>::Out: Log2Ceil,
{ type Out = Su<<<N as Inc>::Out as Log2Ceil>::Out>; }

pub trait Add<R> { type Out; }
impl<R> Add<R> for Z { type Out = R; }
impl<L, R> Add<R> for Su<L> where L: Add<R> { type Out = Su<<L as Add<R>>::Out>; }
...
macro_rules! slots_agree {
    ($c:ty) => {
        const _: () = { assert!(<$c as Store<u8>>::SLOTS == <$c as PosVal>::VAL); };
        const _: () = {
            // and the declaration is tied to something the compiler laid out:
            // one u8 per slot, so the shape's size is the capacity.
            assert!(core::mem::size_of::<<$c as Store<u8>>::Shape>()
                    == <$c as PosVal>::VAL as usize);
        };
    };
}
slots_agree!(C1);  slots_agree!(C2);  slots_agree!(C3);  slots_agree!(C4);
slots_agree!(C5);  slots_agree!(C7);  slots_agree!(C13); slots_agree!(C16);

// ---- ARM S3: a fold over the derived shape into the derived accumulator --
pub struct Num<W>(pub u64, PhantomData<W>);
impl<W> Num<W> { pub const fn new(v: u64) -> Self { Num(v, PhantomData) } }
impl<W> Clone for Num<W> { fn clone(&self) -> Self { Num(self.0, PhantomData) } }
impl<W> Copy for Num<W> {}

pub trait CAdd { fn cadd(self, rhs: Self) -> Self; fn zero() -> Self; }
impl<W> CAdd for Num<W> {
    fn cadd(self, r: Self) -> Self { Num::new(self.0.wrapping_add(r.0)) }
    fn zero() -> Self { Num::new(0) }
}

pub trait SumAccum<C> { type Acc: CAdd + Copy; fn lift(self) -> Self::Acc; }
impl<W, C> SumAccum<C> for Num<W>
where C: Log2Ceil, W: Add<<C as Log2Ceil>::Out>,
{
...
pub trait FoldInto<C, A: CAdd> { fn fold_into(&self, acc: A) -> A; }
impl<W, C, A: CAdd> FoldInto<C, A> for Slot<Num<W>>
where Num<W>: SumAccum<C, Acc = A> {
    fn fold_into(&self, acc: A) -> A { acc.cadd(self.0.lift()) }
}
impl<C, A: CAdd, X: FoldInto<C, A>, Y: FoldInto<C, A>> FoldInto<C, A> for Pair<X, Y> {
    fn fold_into(&self, acc: A) -> A { self.1.fold_into(self.0.fold_into(acc)) }
}

type N4 = Su<Su<Su<Su<Z>>>>;

pub fn s3_sum(store: &<C16 as Store<Num<N4>>>::Shape)
    -> <Num<N4> as SumAccum<C16>>::Acc
where <Num<N4> as SumAccum<C16>>::Acc: CAdd,
...
pub struct NoRow;
#[cfg(arm_s4)]
impl PosVal for NoRow { const VAL: u64 = 9; }
#[cfg(arm_s4)]
pub fn s4_uncovered() -> <NoRow as Store<u8>>::Shape { unimplemented!() }
...
macro_rules! packs_like_an_array {
    ($c:ty) => {
        ...
        const _: () = {
            assert!(core::mem::size_of::<<$c as Store<u64>>::Shape>()
                    == <$c as PosVal>::VAL as usize * core::mem::size_of::<u64>());
        };
        const _: () = {
            assert!(core::mem::align_of::<<$c as Store<u64>>::Shape>()
                    == core::mem::align_of::<u64>());
        };
    };
}
packs_like_an_array!(C1);  packs_like_an_array!(C2);  packs_like_an_array!(C3);
packs_like_an_array!(C4);  packs_like_an_array!(C5);  packs_like_an_array!(C7);
packs_like_an_array!(C13); packs_like_an_array!(C16);
