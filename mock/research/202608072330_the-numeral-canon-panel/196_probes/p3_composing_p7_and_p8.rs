...
impl<W> CAdd for Num<W> {
    fn cadd(self, rhs: Self) -> Self { Num::new(self.0.wrapping_add(rhs.0)) }
...
/// is tested.
pub struct BoundedSlice<'a, T, C> { pub items: &'a [T], pub cap: PhantomData<C> }

pub fn sum_bounded<'a, W, C>(xs: &BoundedSlice<'a, Num<W>, C>) -> <Num<W> as SumAccum<C>>::Acc
where Num<W>: SumAccum<C>,
{
    let mut acc = <<Num<W> as SumAccum<C>>::Acc as CAdd>::zero();
    let mut i = 0usize;
    while i < xs.items.len() { acc = acc.cadd(xs.items[i].lift()); i += 1; }
    acc
}

// ---- ARM A2: the derived widths against integer arithmetic --------------

const fn lg_ref(v: u64) -> u32 { let mut r = 0u32; let mut p = 1u64;
    while p < v { p *= 2; r += 1; } r }
...
type N0 = Z; type N1 = Su<N0>; type N2 = Su<N1>; type N3 = Su<N2>; type N4 = Su<N3>;
...
macro_rules! check_cap {
    ($c:ty) => {
        const _: () = { assert!(<$c as PosVal>::VAL == <$c as PosVal>::VAL); };
        const _: () = {
            // the accumulator width must equal W + ceil(log2 C), computed by
            // integer arithmetic on the capacity's own value.
            assert!(<(Num<N4>, $c) as AccWidth>::W
                    == 4 + lg_ref(<$c as PosVal>::VAL));
        };
    };
}
check_cap!(C1); check_cap!(C2); check_cap!(C3); check_cap!(C4);
check_cap!(C5); check_cap!(C7); check_cap!(C16); check_cap!(C256);
...
pub fn a1_use(xs: &[Num<N4>]) -> Num<N8> {
    let b = BoundedSlice::<Num<N4>, C16> { items: xs, cap: PhantomData };
    sum_bounded(&b)
}
...
#[cfg(arm_b2)]
pub struct BoundedArray<T, C: PosVal> { pub items: [T; C::VAL as usize], pub cap: PhantomData<C> }
