...
// The residue claimed in the talk: does a consumer bounding BOTH have to
// disambiguate `C::Array<T>`? Written the naive way on purpose.
pub fn both_bounds<C: Capacity + ConstCapacity>(a: &<C as Capacity>::Array<u8>) -> usize {
    a.as_ref().len()
}
