...
// Where the delegation does NOT come for free. Inside the impl above, `Self`
// is the concrete `Dim<N>` and the solver finds its ConstCapacity impl right
// there. Here `C` is a type parameter, so nothing establishes it.
pub fn generic_no_bound<C: Capacity>() -> <C as ConstCapacity>::Array<u8> {
