...
// --- the word count: ceil(W / 64), by two disjoint blanket impls -------------
pub trait WordCount {
    /// unary tally of 64-bit words
    type W;
}
...
where
    ...
    <T as Shr6>::Q: ToTally,
{
    type W = <<T as Shr6>::Q as ToTally>::T;
}
// the round-up arm lives in `WordCountUp`, since two blanket impls over the same
// self type need the classifiers to be provably disjoint and rustc reports the
// overlap otherwise. Section note in the file.
impl<T> WordCount for T
where
    T: Shr6,
    <T as Shr6>::R: AnyOne,
    <T as Shr6>::Q: ToTally,
{
    type W = S<<<T as Shr6>::Q as ToTally>::T>;
}
pub trait WordCountUp {
    type W;
}
...
// decrement, so the rung is keyed on the digit count of W - 1. That is what
// separates 8 from 9 and 128 from 129.
pub trait Dec {
    type O;
}
impl<T> Dec for D1<T> {
