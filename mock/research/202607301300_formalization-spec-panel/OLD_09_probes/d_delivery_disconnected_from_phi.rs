...
impl<N: Numeral, P: Policy, L> AddAssoc for Number<N, P, L>
where
...
impl<N: Numeral, P: Policy, L: Lowering> Arith for Number<N, P, L>
where
    OverG<P>: Or<UnderG<P>>,
    L::Delivery: Deliver<OverG<P>>
        + Deliver<UnderG<P>>
        + Deliver<JoinG<P>>
        + LiftD<OverG<P>, JoinG<P>>
        + LiftD<UnderG<P>, JoinG<P>>,
{
    type Answer<T: Copy> = <L::Delivery as Deliver<JoinG<P>>>::C<T>;
    fn ok<T: Copy>(v: T) -> Self::Answer<T> {
    ...
    fn over<T: Copy>(nearest: T) -> Self::Answer<T> {
        ...
        <L::Delivery as LiftD<OverG<P>, JoinG<P>>>::lift(
            <L::Delivery as Deliver<OverG<P>>>::refuse(nearest),
        )
    }
    fn under<T: Copy>(nearest: T) -> Self::Answer<T> {
        <L::Delivery as LiftD<UnderG<P>, JoinG<P>>>::lift(
            <L::Delivery as Deliver<UnderG<P>>>::refuse(nearest),
        )
    }
}
...
pub type UFixed<const I: u16, const F: u16, S> = Number<Fix<I, F, Unsigned>, S, S>;
pub type IFixed<const I: u16, const F: u16, S> = Number<Fix<I, F, Signed>, S, S>;
...
pub fn c5() {
    fold::<Number<Unorm<8>, Hot, Hot>>()
}

// ===========================================================================
// JOB 2 REPRODUCTION: does the runtime arithmetic actually call phi?
// ===========================================================================

fn main() {
    // Hot's OverRange is ReduceModulo (wrap). Under a genuine wraparound
    // recovery map, an out-of-range value should wrap, not clamp.
    // range: [0, 7] (max=7, min=0, matching the witness's own test domain).
    // phi's own computation for ReduceModulo at x=9 (2 over max=7):
    let phi_says = match <ReduceModulo as Resolve>::phi(9, 0, 7) {
        Rec::At(v) => v,
        Rec::Refused => -1,
    };
    println!("ReduceModulo::phi(9, min=0, max=7) = {phi_says}  (wrap-around answer)");

    // now call the REAL arithmetic path with the same policy (Hot, whose
    // OverRange = ReduceModulo) and an out-of-range sum, and see what the
    // Answer carrier actually holds.
    let ans = add::<Number<Fix<3, 0, Unsigned>, Hot, Hot>>(6, 3, 0, 7); // 6+3=9, out of [0,7]
    match ans {
        Total(v) => println!("add() under Hot/ReduceModulo returned Total({v})"),
    }

    println!(
        "phi says wrap gives {phi_says}; the runtime delivery ignores that and returns the caller's hardcoded `max` regardless of which resolution is configured."
    );
}
