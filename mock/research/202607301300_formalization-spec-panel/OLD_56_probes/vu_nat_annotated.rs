//! File 46's completion of the seal: vu_nat_sealed.rs (file 42's fix,
//! Pos and Nat sealed) with ONE further diff, the seal file 41 recommended
//! and file 42 never carried into the composed copy: `Adjustment` gains the
//! same private supertrait, with the seal's blanket impl carrying the
//! identical reduction bound the trait's own impl carries. Probe 1b in this
//! directory compiles the attack this closes (a direct foreign
//! `impl Adjustment` with fabricated NUM/DEN, file 41's probe_4b), which
//! lands clean against the tower as file 42 left it. Everything else in
//! this file is byte-identical to 42_probes/vu_nat_sealed.rs; diff the two
//! to audit.
//!
//! The fix: vu_nat.rs's Pos and Nat, with the private supertrait seal
//! 36_probes/probe_5_sealed_perimeter_lib.rs already demonstrated in
//! isolation, applied here where the composing tower actually lives.
//! Everything else in the file is untouched. Probe 3 verifies this closes
//! both attacks (probe 1's Adjustment widening, probe 2's Bias widening)
//! with zero change to Adjustment, Bias, Gcd, ExactDivOdd, Strip2 or
//! Reduce, and the price sweep in price/ confirms the seal costs nothing
//! measurable.
...
// --- encoding, SEALED: Pos and Nat carry a private supertrait so a
// downstream crate cannot widen either, closing the hole probe 4b/the
// fabricated-Pos attack demonstrates against the composing tower. This
// is 36_probes/probe_5's own seal, applied here where it actually needs
// to be: in the module everything else (Adjustment, Bias, the price
// sweep) really composes with, not in an orphaned standalone demo. ---

mod sealed {
    pub trait PosSealed {}
    pub trait NatSealed {}
    pub trait AdjustmentSealed {}
}
...
#[diagnostic::on_unimplemented(
    ...
    label = "this ratio's numerator or denominator is not a concrete constructor chain",
    note = "`Reduce` recurses through `Strip2`/`Gcd`/`ExactDivOdd` on the pair's own \
            constructors and has no base case for an unconstrained type parameter; \
            name a concrete `Pos` here, or bound this position on the operation's own \
            result numeral instead of on `Reduce` directly"
)]
...
/// convention.
pub trait Adjustment: sealed::AdjustmentSealed {
    const NUM: u64;
    const DEN: u64;
}
impl<N: Pos + Gcd<D, Out = H>, D: Pos> sealed::AdjustmentSealed for Ratio<N, D> {}
