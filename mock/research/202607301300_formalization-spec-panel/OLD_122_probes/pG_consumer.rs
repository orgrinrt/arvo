...
// COST SITE ONE: a concrete consumer. No kind argument anywhere.
pub type UFixed13_3Warm = Number<U13F3, Warm>;
pub type Binary32Warm = Number<Binary32, Warm>;
pub fn concrete_consumer(x: UFixed13_3Warm) -> UFixed13_3Warm {
    x
}

// COST SITE TWO: a downstream crate generic over numeral and preset, which is
// what an algorithm crate writes. This is where the projection is paid.
pub fn generic_consumer<N: Numeral, S>(x: Number<N, S>) -> Number<N, S>
where
    S: Policy<N::Exponent> + Lowering<N::Exponent>,
{
    x
}
