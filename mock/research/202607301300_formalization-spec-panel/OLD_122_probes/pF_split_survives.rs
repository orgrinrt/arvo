...
// Does file 120's E0220 refusal (120:154-164) survive the parameterisation?
// A law bounding on the policy half alone, trying to project a lowering member.
pub struct Fact<N, S>(core::marker::PhantomData<(N, S)>);
pub trait AddAssoc {}
pub trait IsDense {}

impl<N: Numeral, S: Policy<N::Exponent>> AddAssoc for Fact<N, S> where
    <S as Lowering<N::Exponent>>::Layout: IsDense
{
}
