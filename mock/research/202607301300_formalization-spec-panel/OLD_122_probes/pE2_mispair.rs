...
// NEGATIVE TWO: the mispairing probe C admitted at exit 0. A marker serving
// only the fixed-point form, offered with a Ranged numeral. There is no
// argument position to mis-supply, so the only way to attempt it is to give
// the marker no float row and pair it with a float numeral.
pub struct FixedOnly;
const impl<E: Exponent, A: Adjustment, B: Bias> Lowering<Implicit<E, A, B>> for FixedOnly {
    type StoredWidth = DoubleLogical;
    type Layout = Dense;
    type Door = Inert;
    type Container = u32;
}
const impl<E: Exponent, A: Adjustment, B: Bias> Policy<Implicit<E, A, B>> for FixedOnly {
    type Quantisation = QFixedWarm;
}

pub type Nonsense = Number<Binary32, FixedOnly>;

// A type alias does not check its bounds, so the alias above proves nothing.
// This is the same signature probe C compiled at exit 0.
