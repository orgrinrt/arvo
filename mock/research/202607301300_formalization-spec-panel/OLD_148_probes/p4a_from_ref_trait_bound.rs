...
/// both widths grow
pub fn widen_both(a: U<N13, N3>) -> U<N20, N8> {
    (&a).into()
}

/// only the fraction grows
pub fn widen_f_only(a: U<N13, N3>) -> U<N13, N8> {
    (&a).into()
}

/// the reflexive case still belongs to the language, by value, no token
pub fn reflexive(a: U<N13, N3>) -> U<N13, N3> {
    a.into()
}

/// explicit `From::from` also reaches it
pub fn via_from(a: U<N13, N3>) -> U<N20, N8> {
    <U<N20, N8> as From<&U<N13, N3>>>::from(&a)
}

/// reachable from a generic bound with no numeral named
...
pub fn use_generic(a: U<N13, N3>) -> U<N20, N8> {
    generic(&a)
}

/// and from a higher-ranked bound over the numeral itself rather than over the reference,
/// which is what a downstream library would actually want to write
pub fn hrtb<A, B>(a: A) -> B
where
    A: Copy,
    for<'x> &'x A: Into<B>,
{
...
pub fn use_hrtb(a: U<N13, N3>) -> U<N20, N8> {
    hrtb(a)
}

/// a degenerate width is fine on both sides
pub fn from_zero(a: U<N0, N0>) -> U<N1, N1> {
    (&a).into()
}
