...
//!
//! NEGATIVE CONTROL for probe_3c. Expected: a refusal whose headline names
//! the NUMERAL, which probe_3a's projected-tag shape could not do.
//! Otherwise probe_3's shape with one change: host-implementedness is a
//! sealed marker trait ON THE NUMERAL rather than a projected tag type. Same
//! refusal, strictly better diagnostic (probe_3d), and no second impl, so the
//! E0119 of probe_2a never arises: there is nothing to overlap WITH.
...
#[diagnostic::on_unimplemented(
    message = "this target's floating-point unit does not implement the numeral `{Self}`",
...
// cfg-gated per target in the real thing: this is a fact about the silicon.
impl HostImplemented for Binary32 {}
impl Numeral for Binary64 {}
// cfg-gated per target in the real thing: this is a fact about the silicon.
impl HostImplemented for Binary64 {}
impl Numeral for Ranged11Abrupt {}
// deliberately no `impl HostImplemented`. Absence IS the mechanism.
...
/// by a second impl.
impl<N: Numeral + HostImplemented> DefaultLowering<N> for Hot {
