...
// Probe 5a. A fact keyed on the ENCODING, given two faces that denote DIFFERENT
// values. This must refuse, and the question is what the message names.
//
// Expected: it names the expansion (`BPos<H, O<H>>` vs `BPos<H, I<H>>`), not
// the faces. That is file 56's "face decay one hop in", and this probe is here
// to argue it is the CORRECT behaviour at this operation rather than a
// diagnostic defect: the operation is keyed on the encoding, the encodings are
// what differ, so naming them is naming the thing that actually failed.
//
