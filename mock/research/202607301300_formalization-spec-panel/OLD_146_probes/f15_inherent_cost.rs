//! What the inherent `into` costs. With an inherent `into` on the numeral, is a
//! genuine `From<Fixed<..>> for SomeOtherType` still reachable as `.into()`?
#![no_std]
...
// a genuine unrelated conversion the consumer would also want to spell `.into()`
pub struct Degrees(pub u32);
impl<const I: u32, const F: u32, G, S> From<Fixed<I, F, G, S>> for Degrees {
    fn from(_: Fixed<I, F, G, S>) -> Self {
        Degrees(0)
    }
}

pub fn other_target(a: UFixed<13, 3, Warm>) -> Degrees {
    a.into()
}
