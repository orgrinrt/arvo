...
fn main() {
    ...
    // ---- what each losing reading costs the strategy it lands on -----------
    // The source reading: a Precise destination accepts a value its own row refuses.
    let mut precise_polluted: Option<String> = None;
    // The join reading: a Hot destination acquires a refusal, which its row does not have.
    let mut hot_refuses: Option<String> = None;
    for &a in &shapes {
        for &b in &shapes {
            for &sa in &ALL {
                for &sb in &ALL {
                    for j in 0..a.count() {
                        ...
                        if sb == S::Precise
                            && by_tgt.is_none()
                            && by_src.is_some()
                            && precise_polluted.is_none()
                        {
                            precise_polluted = Some(format!(
                    "Q{}.{} {:?} raw {} into Q{}.{} Precise: the target row REFUSES, the source row lands {:?}",
                    a.i, a.f, sa, j, b.i, b.f, by_src));
                        }
                        if sb == S::Hot
                            && by_tgt.is_some()
                            && by_join.is_none()
                            && hot_refuses.is_none()
                        {
                            hot_refuses = Some(format!(
                    "Q{}.{} {:?} raw {} into Q{}.{} Hot: the target row lands {:?}, the join REFUSES",
                    a.i, a.f, sa, j, b.i, b.f, by_tgt));
                        }
                    }
                }
            }
    ...
    if let Some(s) = precise_polluted {
        println!("K. source reading: {}", s);
    }
    if let Some(s) = hot_refuses {
        println!("K. join reading:   {}", s);
    }
}
