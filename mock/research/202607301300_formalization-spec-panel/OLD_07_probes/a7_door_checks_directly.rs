...
// forcing site removed: does the lie still fail?
...
/// The one runtime door to a resolution's semantics. The inline const
/// forces the witness at monomorphisation: any USE of a mis-classified
/// resolution fails the build, even if the declaring crate forgot to force.
pub fn resolve<R: Resolution>(x: i32, min: i32, max: i32) -> i32 {
    const {
        assert!(
            stable::<R>(0, 7, false) == <R::StableOneSided as TruthMarker>::VALUE
                && stable::<R>(-8, 7, true) == <R::StableTwoSided as TruthMarker>::VALUE,
            "this resolution's declared stability disagrees with its own recovery map"
        )
    };
    R::phi(x, min, max)
}
...
fn main() {
    ...
    let v = resolve::<SubstituteZero>(9, 0, 7);
    let _ = v;
