...
/// resolution fails the build, even where the declaring crate forgot its
/// own forcing line. See the panel file for the two-site discipline.
pub fn resolve<R: Resolution>(x: i32, min: i32, max: i32) -> i32 {
    const { <R as Resolution>::WITNESS };
    R::phi(x, min, max)
}

fn main() {
    fold::<ReduceModulo, ReduceModulo, Signed>(); // wrap folds signed
    fold::<Clamp, Clamp, Unsigned>(); // unsigned clamp folds
                                      // fold::<Clamp, Clamp, Signed>();          // refused, correctly
                                      // fold::<SubstituteZero, SubstituteZero, Unsigned>(); // refused: 01 finding 1
    let wrapped = resolve::<ReduceModulo>(9, 0, 7);
    assert_eq!(wrapped, 1);
    let clamped = resolve::<Clamp>(9, 0, 7);
    assert_eq!(clamped, 7);
