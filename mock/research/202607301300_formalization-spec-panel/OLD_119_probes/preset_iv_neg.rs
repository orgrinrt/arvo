...
// negative control: try to key a preset row more finely than the form,
// giving one particular fixed-point numeral its own StoredWidth.
pub struct EOne;
impl Exponent for EOne {}
impl<A: Adjustment, B: Bias> Lowering<Implicit<EOne, A, B>> for Warm {
    type StoredWidth = Minimum;
    type Door = Inert;
}
