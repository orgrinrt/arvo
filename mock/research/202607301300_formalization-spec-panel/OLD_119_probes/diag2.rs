...
#[diagnostic::on_unimplemented(
    ...
    label = "this width has no numeral",
    note = "arvo admits integer and fraction widths 0 through 63; the sum I + F must also lie in that range",
    note = "widen the strategy or split the value across two numbers"
)]
pub trait NatIndex {
    type Out: Nat;
}
...
pub struct Number<N, S>(core::marker::PhantomData<(N, S)>);
pub struct Warm;
pub type UFixed<const A: u16, const B: u16, S> = Number<PrecisionOf<A, B>, S>;
pub fn consumer(_x: UFixed<200, 3, Warm>) {}
