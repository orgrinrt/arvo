//! probe_7: the layered-crate positive control. This is
//! vu_bias_sealed_adj.rs with ONE diff: the nat tower arrives as a
//! genuinely separate, separately compiled crate (`vu_nat_layer`, an
//! rlib) rather than a #[path] module. If this compiles, the design's own
//! layered-crate shape crosses the seal for its good reason: a downstream
//! layer CAN declare a new guarantee-carrying trait (`Bias`), with its
//! own seal, whose blanket impl bounds on the upstream sealed carriers,
//! without the upstream seal being open to it in any way. The seal
//! quantifies over inhabitant introduction, not over composition, and
//! this is that sentence compiled.
//!
//! Compiled as:
//!   rustc --edition 2021 --crate-type lib --crate-name vu_nat_layer vu_nat_sealed_adj.rs
//!   rustc --edition 2021 --crate-type lib --extern vu_nat_layer=libvu_nat_layer.rlib \
//!     probe_7_bias_as_separate_crate.rs
//!
