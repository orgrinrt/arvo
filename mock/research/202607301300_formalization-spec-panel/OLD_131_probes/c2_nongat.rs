...
pub trait StoreFor<const N: u32> {
    type T: Copy;
}
pub struct Hot;
impl<const N: u32> StoreFor<N> for Hot
where
    Picker: Project<{ <Rung<N> as Tagged>::TAG }>,
{
    type T = <Picker as Project<{ <Rung<N> as Tagged>::TAG }>>::T;
}
pub fn a(x: <Hot as StoreFor<3>>::T) -> u8 {
    x
}
pub fn b(x: <Hot as StoreFor<30>>::T) -> u16 {
    x
}
