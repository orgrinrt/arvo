...
// ---------------------------------------------------------------- sign axis
pub trait Sign: Copy { const EXTRA: u32; }
#[derive(Clone, Copy)] pub struct Unsigned;
#[derive(Clone, Copy)] pub struct Signed;
impl Sign for Unsigned { const EXTRA: u32 = 0; }
impl Sign for Signed   { const EXTRA: u32 = 1; }

// -------------------------------------------------- the ladder the hw has
#[derive(Clone, Copy)] pub struct Wide<const BYTES: usize, A>([u8; BYTES], PhantomData<A>);
#[derive(Clone, Copy)] pub struct A1;
#[derive(Clone, Copy)] pub struct A16;

pub trait Project<const TAG: usize, G: Sign, const BYTES: usize, S> { type T: Copy; }
pub struct Picker;
pub struct Hot; pub struct Cold; pub struct Warm; pub struct Precise;
...
impl<G: Sign, const B: usize> Project<5, G, B, Hot>     for Picker { type T = Wide<B, A16>; }
impl<G: Sign, const B: usize> Project<5, G, B, Cold>    for Picker { type T = Wide<B, A1>;  }
impl<G: Sign, const B: usize> Project<5, G, B, Warm>    for Picker { type T = Wide<B, A1>;  }
impl<G: Sign, const B: usize> Project<5, G, B, Precise> for Picker { type T = Wide<B, A1>;  }

pub const fn tag_min(w: u32) -> usize {
    if w<=8 {0} else if w<=16 {1} else if w<=32 {2} else if w<=64 {3} else if w<=128 {4} else {5}
}
pub const fn tag_headroom(w: u32) -> usize {
    if w<=8 {0} else if w<=16 {1} else if w<=32 {2} else if w<=64 {3} else {5}
}
pub const fn bytes_for(w: u32) -> usize { (w as usize).div_ceil(8) }
...
tagged!(Hot, tag_min); tagged!(Cold, tag_min);
tagged!(Warm, tag_headroom); tagged!(Precise, tag_headroom);

/// The container level: derived, never declared as an axis.
pub trait Store<const I: u32, const F: u32, G: Sign> { type T: Copy; }
macro_rules! store {
    ($s:ty) => {
        impl<const I: u32, const F: u32, G: Sign> Store<I, F, G> for $s
        where Picker: Project<{ <Rung<I,F,G,$s> as Tagged>::TAG }, G,
                              { <Rung<I,F,G,$s> as Tagged>::BYTES }, $s>,
        {
            type T = <Picker as Project<{ <Rung<I,F,G,$s> as Tagged>::TAG }, G,
                                        { <Rung<I,F,G,$s> as Tagged>::BYTES }, $s>>::T;
        }
    };
}
store!(Hot); store!(Cold); store!(Warm); store!(Precise);
...
pub struct ProductFormat<const I: u32, const F: u32, const J: u32, const K: u32,
...
pub fn add<const I: u32, const J: u32, const F: u32, const M: u32, G: Sign, S>(
    _a: Fixed<I, F, G, S>, _b: Fixed<J, F, G, S>,
) -> Fixed<M, F, G, S>
where S: Store<I, F, G> + Store<J, F, G> + Store<M, F, G>,
{
...
pub fn mul<const I: u32, const F: u32, const J: u32, const K: u32,
           const M: u32, const N: u32, G: Sign, S>(
    _a: Fixed<I, F, G, S>, _b: Fixed<J, K, G, S>,
) -> Fixed<M, N, G, S>
...
where S: Store<I, F, G> + Store<J, K, G>,
{ unimplemented!() }
...
where S: Store<I, F, G> + Store<J, F, G>,
{
    const { assert!(true) }
