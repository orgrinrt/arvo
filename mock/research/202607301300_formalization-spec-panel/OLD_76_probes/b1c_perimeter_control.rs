//! B1c. PERIMETER CONTROL. As B1b, but the offending capacity is one the
//! file's assertion list never names, reached only through `as_slice`. If the
//! door's own inline const block is doing the work, this still refuses; if the
//! discharge lived only in the macro list, this compiles and the `unsafe` is
//! open for every capacity a downstream crate invents.
//!
//! B1b. NEGATIVE CONTROL for B1: one byte of padding is introduced into the
//! odd-arity node, and the layout law must refuse. If it does not, every
//! assertion in B1 was vacuous and its `unsafe` is undischarged.
//!
...
#[repr(C)]
pub struct TwicePlus<A, T>(A, A, T, u8);
...
layout_law! {
    N0 => 0, N1 => 1, N64 => 64, N4096 => 4096,
}

// A capacity invented downstream, in no assertion list anywhere, odd, and so
// laid out through the corrupted node. Only the door can catch this.
pub type N11 = Pz<I<I<O<H>>>>;
pub fn downstream(s: &<N11 as Nat>::Store<u32>) -> u32 {
    sum_generic::<N11>(s)
}
