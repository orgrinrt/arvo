...
emit!( [x x x x x x x] ; H ); // widths 1 ..= 31
...
pub type UFixedAt<Iw, Fw, S> = Number<FixedNumeral<Sum<Iw, Fw>, NonNegative>, S>;
...
// Spelling D-bin: binary digits, a direct transcription with no arithmetic at all.
#[macro_export]
macro_rules! b {
    (@ $acc:ty ; ) => { $crate::Pz<$acc> };
    (@ $acc:ty ; 0 $($r:tt)* ) => { $crate::b!(@ $crate::O<$acc> ; $($r)*) };
    (@ $acc:ty ; 1 $($r:tt)* ) => { $crate::b!(@ $crate::I<$acc> ; $($r)*) };
    ( 0 ) => { $crate::Z };
    ( 1 $($r:tt)* ) => { $crate::b!(@ $crate::H ; $($r)*) };
}
