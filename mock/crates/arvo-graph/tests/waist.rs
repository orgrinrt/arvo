...
fn hourglass_waist_detected() {
    ...
    let mut dag: BitMatrix<Bits<64, Hot, Unsigned>, C5> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
...
fn no_waist_when_width_monotone() {
    ...
    let mut dag: BitMatrix<Bits<64, Hot, Unsigned>, C6> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
    dag.set_edge(nid(0), nid(1));
    dag.set_edge(nid(0), nid(2));
