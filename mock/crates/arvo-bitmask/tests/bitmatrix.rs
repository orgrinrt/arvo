...
use arvo::{Cap, USize};
use arvo_bitmask::{BitMatrix, BitMatrix, NodeId};
...
fn empty_matrix_has_no_edges() {
    let m: BitMatrix<Bits<64, Hot, Unsigned>, C8> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
    for i in 0..8 {
        for j in 0..8 {
...
fn set_edge_then_edge_reads_true() {
    let mut m: BitMatrix<Bits<64, Hot, Unsigned>, C8> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
...
fn transitive_closure_chain() {
    ...
    let mut m: BitMatrix<Bits<64, Hot, Unsigned>, C4> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
    m.set_edge(nid(0), nid(1));
    m.set_edge(nid(1), nid(2));
...
fn transitive_closure_fan_out() {
    // 0 -> 1, 0 -> 2; 1 -> 3; 2 -> 4.
    let mut m: BitMatrix<Bits<64, Hot, Unsigned>, C5> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
    m.set_edge(nid(0), nid(1));
...
fn out_of_range_edge_is_false() {
    let m: BitMatrix<Bits<64, Hot, Unsigned>, C4> = BitMatrix<Bits<64, Hot, Unsigned>>::empty();
    assert!(!*m.edge(nid(10), nid(0)));
    assert!(!*m.edge(nid(0), nid(10))); // bit index beyond width is still false
}

// --- BitMatrix<Bits<256, Hot, Unsigned>> -------------------------------------------------------

#[test]
fn bitmatrix256_edge_set_clear() {
    let mut m: BitMatrix<Bits<256, Hot, Unsigned>, C4> = BitMatrix<Bits<256, Hot, Unsigned>>::empty();
