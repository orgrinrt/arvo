...
{
    let n = cap_size(N);
    let mut v: [F; cap_size(N)] = [F::from_constant(1); cap_size(N)];

    let mut step = 0usize;
    while step < iterations.0 {
        // v_new = matrix * v (dense matrix-vector product).
        let mut next: [F; cap_size(N)] = [F::from_constant(0); cap_size(N)];
        let mut i = 0usize;
        while i < n {
            let mut acc = F::from_constant(0);
            let mut j = 0usize;
            while j < n {
        ...
        let mut sq_sum = F::from_constant(0);
        let mut k = 0usize;
        while k < n {
