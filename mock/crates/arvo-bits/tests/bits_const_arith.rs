...
use arvo_strategy::{Additive, Bounded, Hot, Identity, Multiplicative, Signed, Unsigned, Warm};

const _BITS8_ZERO: Bits<8, Hot, Unsigned> =
    <Bits<8, Hot, Unsigned> as Identity<Additive>>::IDENTITY;
const _BITS8_ONE: Bits<8, Hot, Unsigned> =
    <Bits<8, Hot, Unsigned> as Identity<Multiplicative>>::IDENTITY;
...
const _BITS16_ZERO: Bits<16, Hot, Unsigned> =
    <Bits<16, Hot, Unsigned> as Identity<Additive>>::IDENTITY;
const _BITS32_ZERO: Bits<32, Hot, Unsigned> =
    <Bits<32, Hot, Unsigned> as Identity<Additive>>::IDENTITY;
const _BITS64_ZERO: Bits<64, Hot, Unsigned> =
    <Bits<64, Hot, Unsigned> as Identity<Additive>>::IDENTITY;
const _BITS128_ZERO: Bits<128, Hot, Unsigned> =
    <Bits<128, Hot, Unsigned> as Identity<Additive>>::IDENTITY;
const _BITS256_ZERO: Bits<256, Warm, Unsigned> =
    <Bits<256, Warm, Unsigned> as Identity<Additive>>::IDENTITY;

const _BITS_SIGNED_ZERO: Bits<32, Hot, Signed> =
    <Bits<32, Hot, Signed> as Identity<Additive>>::IDENTITY;
const _BITS_SIGNED_ONE: Bits<32, Hot, Signed> =
    <Bits<32, Hot, Signed> as Identity<Multiplicative>>::IDENTITY;

const _BITS_BIT_ACCESS_PROBE: () = {
    type B = Bits<32, Hot, Unsigned>;
    let zero = <B as Identity<Additive>>::IDENTITY;
...
fn bits_const_runtime_parity() {
    ...
    let zero = <B32 as Identity<Additive>>::IDENTITY;
    ...
    let one = <B64 as Identity<Multiplicative>>::IDENTITY;
