//! `BitWidth::WIDTH` correctness across types and strategies.
...
use arvo_bits::{BitWidth, Byte, DWord, Nibble, QWord, Word};

#[test]
fn ufixed_width_equals_i_plus_f() {
    // `UFixed<IBits(3), FBits(5), Hot>` -> 8 logical bits.
    assert_eq!(
        <UFixed<{ IBits(3) }, { FBits(5) }, Hot> as BitWidth>::WIDTH.0,
        8
    );
    // `UFixed<IBits(12), FBits(4), Warm>` -> 16 logical bits.
    assert_eq!(
        <UFixed<{ IBits(12) }, { FBits(4) }, Warm> as BitWidth>::WIDTH.0,
        16
    );
    // `UFixed<IBits(32), FBits(0), Cold>` -> 32 logical bits.
    assert_eq!(
        <UFixed<{ IBits(32) }, { FBits::ZERO }, Cold> as BitWidth>::WIDTH.0,
        32
    );
    // `UFixed<IBits(48), FBits(16), Precise>` -> 64 logical bits.
    assert_eq!(
        <UFixed<{ IBits(48) }, { FBits(16) }, Precise> as BitWidth>::WIDTH.0,
        64
    );
}
...
fn ifixed_width_adds_sign_bit() {
    ...
    assert_eq!(
        <IFixed<{ IBits(3) }, { FBits(4) }, Hot> as BitWidth>::WIDTH.0,
        8
    );
    // `IFixed<IBits(15), FBits(0), Warm>` -> 1 + 15 + 0 = 16.
    assert_eq!(
        <IFixed<{ IBits(15) }, { FBits::ZERO }, Warm> as BitWidth>::WIDTH.0,
        16
    );
    // `IFixed<IBits(31), FBits(32), Precise>` -> 1 + 31 + 32 = 64.
    assert_eq!(
        <IFixed<{ IBits(31) }, { FBits(32) }, Precise> as BitWidth>::WIDTH.0,
        64
    );
}
