...
use crate::apply::{self, Dither, Exact, Fraction, Rounded, adapt};
...
fn a_remainder_outside_the_unit_interval_is_normalised_into_the_slot() {
    ...
    assert!(
        Exact::between(Slot::ZERO, Fraction::of(8, 4))
            .is_on_grid()
            .get()
    );
    ...
    assert!(
        Exact::between(Slot::at(7), Fraction::of(3, 0))
            .is_on_grid()
            .get()
    );
    assert_eq!(
