//! `lanes16` with the reduction length lifted to a compile-time constant. One
//! static lever priced on its own, with everything else held.
//!
...
fn run_lanes16_constl<const KEY: usize>(
    input: &<Case<KEY> as Routine>::Input,
    output: &mut <Case<KEY> as Routine>::Output,
) -> FfiBenchCall {
    timed! {
        run {
            output.value = arms::lanes16_constl(KEY, input);
        }
    }
}
