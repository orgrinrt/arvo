...
//! independent saturating accumulators.
//!
//! This arm exists to attack the mechanism that makes the minimum container
//! lose at arity, rather than to report that it loses. See
//! `bench-warm-clamp-shared` for the legality argument and the transforms.

use bench_warm_clamp_shared::{arms_lanes, Case};
use mockspace_bench_core::{timed, FfiBenchCall, Routine};
use mockspace_bench_macro::bench_variant;

#[bench_variant(
    Case,
    "warm-clamp-min-lanes",
    sizes = [
...
fn run_min_lanes<const KEY: usize>(
    input: &<Case<KEY> as Routine>::Input,
    output: &mut <Case<KEY> as Routine>::Output,
) -> FfiBenchCall {
    timed! {
        run {
            output.value = arms_lanes::min_lanes(KEY, input);
        }
    }
}
