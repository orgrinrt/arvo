//! `80`'s `sat_sum_lanes`: the law used correctly, the bounds left unprovable.
//! This is the arm that lost, and it is here because it lost.
//!
...
fn run_lanes4_idx<const KEY: usize>(
    input: &<Case<KEY> as Routine>::Input,
    output: &mut <Case<KEY> as Routine>::Output,
) -> FfiBenchCall {
    timed! {
        run {
            output.value = arms::lanes4_idx(KEY, input);
        }
    }
}
