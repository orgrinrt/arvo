...
//! and what severity a finding actually carries ([`assert_findings_block`] for
//! a lint that refuses, [`assert_findings_carry_at`] for one that names another
//! severity on purpose). The arms asking whether every lint asks these are in
//! `canon_lint_testkit/tests.rs`.
//!
...
#[path = "canon_lint_testkit/tests.rs"]
mod tests;
...
/// The helpers that answer what severity a lint's findings carry. A lint's own
/// tests name one of them, and the arm in `canon_lint_testkit/tests.rs` asking
/// whether each lint reaches the gate looks for either.
const SEVERITY_ASKED: [&str; 2] = ["assert_findings_block", "assert_findings_carry"];
...
pub fn assert_findings_block_at(lint: &dyn RepoLint, ctx: &RepoContext) {
    assert_findings_carry_at(lint, ctx, Severity::HARD_ERROR);
}

/// Every finding this lint produced carries `severity`, and there is at least
/// one.
///
/// What a lint that warns on purpose calls instead of
/// [`assert_findings_block_at`], naming the severity it chose, so the choice is
/// a line in its own tests rather than the absence of one. The body is the one
/// the blocking helper runs.
pub fn assert_findings_carry_at(lint: &dyn RepoLint, ctx: &RepoContext, severity: Severity) {
    ...
    for e in &found {
        assert_eq!(
            e.severity, severity,
            "`{}` reported `{}` at a severity other than the one it names",
